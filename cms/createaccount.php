          ...
          <!-- end: Left Menu -->
            ...
            <div id="content">
			  ...
			  <div class="row" style=" margin-top:10px; margin-right:10px; padding-left: 20px; " id="DontPrint">
                ...
                <div class="col-md-6">
                  <div class="panel">
                    <div class="panel-heading" style="background:#ccc;"><h3>Members with no account</h3></div>
                    <div class="panel-body">
                      <div class="responsive-table">
					 <?php  
				if(!empty($_POST['username']) && !empty($_POST['memberid'])){
...
?>
					 ...
					 <?php
										...
										$tablequery="SELECT * FROM member WHERE memberid NOT IN (SELECT memberid FROM accounts) ";
										$tableresult = mysqli_query($process->connectDB(),$tablequery);
		                                $tablerows=$process->queryrow($tablequery);  
		                                if($tablerows>0){
						...
						<td><img src="form/img/img_member/<?php echo $tablerows['picture']; ?>" style="width:40px; height:40px; border-radius:50%;"/></td>
						  ...
						  <div id="message"></div>
						   <input type="hidden" value="<?php echo $tablerows['memberid']; ?>" name="memberid"/>
						  <input type="hidden" value="<?php echo $tablerows['firstname']; ?>" name="username"/>
						  <input type="hidden" value="<?php echo $tablerows['lastname']; ?>" name="password"/>
						  <input type="submit" name="submit" class="btn btn-primary" value="Make User" /></div> 
						  </form>
			  ...
			  <div class="col-md-6">
                  <div class="panel">
                    <div class="panel-heading" style="background:green;"><h3 style=" color:#fff;">Members with account</h3></div>
                    <div class="panel-body">
                      <div class="responsive-table">
					 <?php  
...
?>
					 ...
					 <?php
										...
										$tablequerymem="SELECT * FROM member WHERE memberid IN (SELECT memberid FROM accounts)";
										...
										$tablequeryacc="SELECT * FROM accounts WHERE memberid ='".$tablerowsmem['memberid']."'";
						...
						<td><img src="form/img/img_member/<?php echo $tablerowsmem['picture']; ?>" style="width:40px; height:40px; border-radius:50%;"/></td>
                          <td><?php echo $tablerowsmem['lastname']." ".$tablerowsmem['firstname']; ?></td>
                          <td>
						  <div>
						   <a href="assignuser.php?accountid=<?php echo $tablerowsacc['accountid']; ?>&&memberid=<?php echo $tablerowsacc['memberid']; ?>" id="show"  class="btn btn-default"> Privileges</a>
