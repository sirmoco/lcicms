          ...
          <!-- end: Left Menu -->
            ...
            <div id="content">
			  ...
			  <div style="float:left; margin-top:10px; margin-right:10px; padding-left: 20px;" id="DontPrint">
                ...
                <div class="col-md-12">
                  <div class="panel">
                    <div class="panel-heading"><h3>Member Absent list</h3></div>
                    <div class="panel-body">
                      <div class="responsive-table">
					
					<div class="panel-content" id="show" style="display:none;">
					<form method="post" id="response">
					<div class="row" >
					
					<div class="col-md-8">
						<textarea  name="response" class="form-text" style="width:100%; border:2px solid #FECE1A; height:200px; "></textarea>
						</div>
						<div class="col-md-2" >
						
						<input class="form-text" type="hidden" name="visitorid" value="<?php echo $_SESSION['memberid']; ?>">
						<input class="form-text" type="hidden" id="memid" name="visitedid" value="">
					 ...
					 <?php
										...
										$tablequery1='SELECT * FROM assignment WHERE shepherd="'.$_SESSION['memberid'].'" && sheep NOT IN(SELECT service_attendance.memberid FROM service_attendance WHERE service_attendance.date = CURRENT_DATE()) && sheep NOT IN(SELECT visitation.memberid FROM visitation WHERE visitation.date = CURRENT_DATE())';
						...
						<td><img src="img/img_member/<?php echo $tablerows['picture']; ?>" style="width:40px; height:40px; border-radius:50%;"/></td>
						  ...
						  <td>
						  <?php
						  if(!empty($tablerows['lng']) && !empty($tablerows['lat'])){
						  ?>
						  <a href="../loca.php?memberid=<?php echo $tablerowsmem['memberid']; ?>&&url=<?php echo $_SERVER['PHP_SELF'];?>" id="show"  class="btn btn-default">Update Location</a>
						 <?php
						  }else{
							  ?>
							<a href="../loca.php?memberid=<?php echo $tablerowsmem['memberid']; ?>&&url=<?php echo $_SERVER['PHP_SELF'];?>" id="show"  class="btn btn-default">Set Location</a>
						   <?php
						  }
						 ?>
						  </td>
						  ...
						  <input type="hidden" name="memid" value="<?php echo $tablerows['memberid']; ?>">
						  <input type="hidden" name="accountid" value="<?php echo $_SESSION['accountid']; ?>">
						  <input type="submit" name="submit" class="btn btn-primary" value="Visit"/>
						  </form>
						  <i  id="check" style="display:none;" class="fa fa-check-square-o"></i>
						  </div>
						  </td>
          ...
          var press = $(this);					 
			  ...
			  success: function(response){
					 ...
					 $('#message').text(response);
					  ...
					  $('#datatables-example').css('display','');
					 }		   		   
 ...
 var message = $(this).find('#message');	
          var press = $(this);	
			  ...
			  success: function(response){
					 ...
					 $('#message').text(response);
					  $('#datatables-example').css('display','none');
					  $('#show').css('display','');
					   $('#memid').val(response);
					 }		   		   
