            ...
            <div id="content">
			  ...
			  <div style="float:left; margin-top:10px; margin-right:10px; padding-left: 20px;" id="DontPrint">
                ...
                <div class="col-md-12">
				  ...
				  if(!empty($_SESSION['administrator'])){
					  $tablequery="SELECT * FROM member ORDER BY memberid DESC";
				  }else{
					  $tablequery="SELECT * FROM member WHERE memberid IN(SELECT sheep FROM assignment WHERE shepherd = '".$_SESSION['memberid']."' ORDER BY sheep DESC )";
		                                ...
		                                if($branchrows>0){
											$branchquery2="SELECT * FROM branch WHERE branch_id='".$tablerows['branch_id']."'";
										$branchresult2 = mysqli_query($process->connectDB(),$branchquery2);
											$branchrows2=mysqli_fetch_assoc($branchresult2);
										
		                                if($branchrows2>0){
									 $branch =	$branchrows2['branch_name'];
		                                }else{
		                                $branch = ' Not assigned to a branch ';
		                                }}
										  ?>  
						<tr>
						<td><a href="member_detail.php?memberid=<?php echo $tablerows['memberid']; ?>&&member_detail=<?php echo $tablerows['lastname'].'_'.$tablerows['firstname']; ?>"><img src="form/img/img_member/<?php echo $tablerows['picture']; ?>" style="width:40px; height:40px; border-radius:50%;"/></a></td>
                          <td><a href="member_detail.php?memberid=<?php echo $tablerows['memberid']; ?>&&member_detail=<?php echo $tablerows['lastname'].'_'.$tablerows['firstname']; ?>"><?php echo $tablerows['salutation']." ".$tablerows['lastname']." ".$tablerows['firstname']; ?></a></td>
                          <td><?php echo $tablerows['mobile1']; ?></td>
                          <td><?php echo $tablerows['residence'];  ?></td>
                          <td><?php echo $tablerows['dob']; ?></td>
                         
                          <td><?php echo $branch; ?></td>
                          <td> <a class="btn btn-default" href="loca.php?memberid=<?php echo $tablerows['memberid']; ?>"> Loc <span class="fa fa-map" style="color:#337ab7;"></span></a>
						  </td>
						  <td>
						       <?php
						    if(!empty($_SESSION['administrator'])){
						  ?>
						       <a href="form/memberregistrationform2.php?memberid=<?php echo $tablerows['memberid']; ?>" class="btn btn-primary"> Edit</a>
						        <?php
							}else{
								?>
