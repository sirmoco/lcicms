	   ...
	   -moz-box-shadow: 0 8px 6px -6px black;
		...
		$tablequeryhead3="SELECT * FROM member WHERE memberid='".$head."'";
				  ...
				  <h5 style="color:green;">'.$tablerowshead3['firstname'].' '.$tablerowshead3['lastname'].' is now '.$name.' ministry head</h5>
					...
					echo '<center>Create Ministry </center>';
					
				}


?>
					  <div class="panel-heading" style="background:#ccc;"><h5 style="color:#000; ">CREATE NEW MINISTRY</h5></div>
					  <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
 					  <div class="form-group form-animate-text col-md-12" style="margin-top:10px !important;">
							...
							<div class="form-group form-animate-text col-md-12" style="margin-top:10px !important;">
							  ...
							  <select class="select2-A" name="head">
                          <option>Select Ministry Head</option>
						   <?php
										
										$tablequeryhead="SELECT * FROM member WHERE memberid IN(SELECT memberid FROM accounts WHERE ministry_head = 1)";
										$tableresulthead = mysqli_query($process->connectDB(),$tablequeryhead);
		                                $tablerowshead=$process->queryrow($tablequeryhead);  
		                                if($tablerowshead>0){
											
										while($tablerowshead=mysqli_fetch_assoc($tableresulthead)) {
										
										
										  ?> 
										  
                            <option value="<?php echo $tablerowshead['memberid']; ?>"><?php echo $tablerowshead['firstname'].' '.$tablerowshead['lastname']; ?></option>
							<?php
										}}
							?>
                          </optgroup>
							...
							<div class="form-group form-animate-text col-md-6" style="margin-top:10px !important;">
							 <input type="submit" name="submitmin" class="btn btn-success" style="display:;" value="Create Ministry" /></div> 
		...
		VALUES(NULL,'".$shepherd."','".$sheep."','".$date_uploaded."')" ;
				...
				if($result){
				  ...
				  echo '<center><h5 style="color:green;">Success! You have assigned '.$tablerows3['lastname'].' '.$tablerows3['firstname'].' to '.$tablerows2['firstname'].'</h5></center>';
					...
					echo '<center>Assign Member To </center>';
...
?>
					 ...
					 <?php
										...
										$tablequery="SELECT * FROM ministry";
										$tableresult = mysqli_query($process->connectDB(),$tablequery);
		                                $tablerows=$process->queryrow($tablequery);  
		                                if($tablerows>0){
											
										while($tablerows=mysqli_fetch_assoc($tableresult)) {
										$tablequerymem3="SELECT * FROM member WHERE memberid='".$tablerows['head']."'";
										$tableresultmem3 = mysqli_query($process->connectDB(),$tablequerymem3);
		                                $tablerowsmem3=$process->queryrow($tablequerymem3);  
		                               
											
										$tablerowsmem3=mysqli_fetch_assoc($tableresultmem3);
										$tablequerymin="SELECT * FROM ministry_member WHERE ministry_id='".$tablerows['ministry_id']."'";
										$tableresultmin = mysqli_query($process->connectDB(),$tablequerymin);
		                                $tablerowsmin=$process->queryrow($tablequerymin); 
$num=mysqli_num_rows($tableresultmin);	
										  ?>  
						<tr>
						
                          <td> <a href="addtoministry.php?ministry_id=<?php echo $tablerows['ministry_id']; ?>&&name=<?php echo $tablerows['name']; ?>"><h5 style="color:#0f9ec0;"><b><?php echo $tablerows['name']; ?></b></h5></a></td>
                          <td><img src="form/img/img_member/<?php echo $tablerowsmem3['picture']; ?>" style="width:50px; border-radius:50%;"/></td>
						  
                          <td><a href="member_detail.php?memberid=<?php echo $tablerowsmem3['memberid']; ?>&&member_detail=<?php echo $tablerowsmem3['lastname'].'_'.$tablerowsmem3['firstname']; ?>"><?php echo $tablerowsmem3['firstname'].' '.$tablerowsmem3['lastname']; ?></a></td>
						  <td>
						  <?php echo $num.' members'; ?>
						  </td>
                       <td><div>
						  
						   <a href="addtoministry.php?ministry_id=<?php echo $tablerows['ministry_id']; ?>&&name=<?php echo $tablerows['name']; ?>" id="show"  class="btn btn-primary">Ministry Members</a>
						 </div> 
						 <?php
						...
						?>
						 <div>
						  <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>?memberid=<?php echo $tablerows['ministry_id']; ?>" >
						  <div id="message"></div>
						   
						  <input type="submit" name="submit" class="btn btn-default" style="display:;" value="Edit" /> 
						  </form>
						  </div>
						  <?php
						 }else{
                ...
                <div class="col-md-12">
                  <div class="panel">
                    ...
                    <div class="panel-heading" style=""><h3>My Ministries</h3></div>
                    <div class="panel-body">
                      <div class="responsive-table">
					 <?php  
...
?>
					 ...
					 <?php
										...
										$tablequery="SELECT * FROM ministry WHERE head='".$_SESSION['memberid']."' || assistant='".$_SESSION['memberid']."'";
										$tableresult = mysqli_query($process->connectDB(),$tablequery);
		                                $tablerows=$process->queryrow($tablequery);  
		                                if($tablerows>0){
...
$num=mysqli_num_rows($tableresultmin);	
										  ?>  
						<tr>
						<td><img src="asset/img/mini.jpg" style="width:40px; height:40px; border-radius:50%;"/></td>
                          <td> <a href="addtoministry.php?ministry_id=<?php echo $tablerows['ministry_id']; ?>&&name=<?php echo $tablerows['name']; ?>"><h5 style="color:#0f9ec0;"><b><?php echo $tablerows['name']; ?></b></h5></a></td>
                          <td><img src="form/img/img_member/<?php echo $tablerowsmem3['picture']; ?>" style="width:50px; border-radius:50%;"/></td>
                       ...
                       <td><div>
						...
						?>
						 <div>
						  <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>?memberid=<?php echo $tablerowss['ministry_id']; ?>" >
						  <div id="message"></div>
